			...
			loaded_data, key_info = CustomWriteQueueAsync(
				function() -- Callback
					if is_get_call == true then
						...
						if version ~= nil then
							local inner_success, inner_error_message = pcall(function()
								get_data, get_key_info = profile_store._global_data_store:GetVersionAsync(profile_key, version)
							end)
							if inner_success == false and type(inner_error_message) == "string" and string.find(inner_error_message, "not valid") ~= nil then
